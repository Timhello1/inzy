...
class MemoryScanMedium : AppCompatActivity() {

    private lateinit var textViewSequence: TextView
    private lateinit var layout: LinearLayout
    private lateinit var buttonUnderstood: Button

    private val elements = listOf('A', 'B', 'C', 'D', 'E', 'F', 'G','H','I','J','K','L','M','N','O','P','Q','R')
    ...
    private fun generateRandomSequence(): String {
        ...
        return shuffledList.subList(0, 6).joinToString("")
    }
    private fun saveScore(){
        val user = auth.currentUser
        val userEmail = user?.email ?: "unknown"

        val scoresCollection = firestore.collection("MemoryScan6")
