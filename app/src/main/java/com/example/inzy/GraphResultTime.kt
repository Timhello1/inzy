...
class GraphResultTime : AppCompatActivity() {
    private val firestore: FirebaseFirestore = FirebaseFirestore.getInstance()
    private val auth: FirebaseAuth = FirebaseAuth.getInstance()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_graph_result_time)
                ...
                .addOnSuccessListener { querySnapshot ->
                    ...
                    if (sortedUserData.isNotEmpty()) {
                        ...
                        for ((index, item) in sortedUserData.withIndex()) {
                            ...
                            val yValue = item["time"] as String

                            if (xValue != null) {
                                lineSeries.appendData(DataPoint(xValue, yValue.toDouble()), true, index + 1)
                                pointSeries.appendData(DataPoint(xValue, yValue.toDouble()), true, index + 1)
                            } else {
                        ...
                        pointSeries.setOnDataPointTapListener() { series, dataPoint ->
                            ...
                            val builder = AlertDialog.Builder(this@GraphResultTime)
