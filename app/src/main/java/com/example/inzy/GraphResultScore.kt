...
class GraphResultScore : AppCompatActivity() {
    private val firestore: FirebaseFirestore = FirebaseFirestore.getInstance()
    private val auth: FirebaseAuth = FirebaseAuth.getInstance()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_graph_result_score)
            ...
            fetchData(collectionName)
                .addOnSuccessListener { querySnapshot ->
                    val userData = querySnapshot.documents.map { document ->
                        ...
                        val time = document.getDouble("score") ?: 0
                    ...
                    if (sortedUserData.isNotEmpty()) {
                        ...
                        for ((index, item) in sortedUserData.withIndex()) {
                            ...
                            val yValue = item["time"] as Double

                            if (xValue != null) {
                                lineSeries.appendData(DataPoint(xValue, yValue), true, index + 1)
                                pointSeries.appendData(DataPoint(xValue, yValue), true, index + 1)
                            } else {
                        ...
                        pointSeries.setOnDataPointTapListener() { series, dataPoint ->
                            ...
                            val builder = AlertDialog.Builder(this@GraphResultScore)
